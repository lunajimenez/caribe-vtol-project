"""
T1. Modelar dinámica de rotación (ecuaciones de Euler) para roll, pitch y yaw.

Implementar el sistema de ecuaciones rotacionales que describe el movimiento angular de la aeronave. 
Esto implica crear las ecuaciones que gobiernan cómo cambian las velocidades angulares según los momentos aplicados. 
...
El cuadricóptero tiene cuatro rotores: frontal (1), lateral derecho (2), 
posterior (3) y lateral izquierdo (4). Los  movimientos  del  cuadricóptero  son  de    rotación  
y  traslación,  determinados  a  tres  ejes fijos (X, Y, Z) y a través de la oscilación de los ángulos 
de navegación: yaw (eje Z); pitch  (eje  Y),  y  roll  (eje  X).  El  cuadricóptero tiene seis grados 
de libertad: tres traslacionales (x, y, z) y tres rotacionales (roll (𝜙), pitch (𝜃) y yaw (𝜓)). 

ROTACIONES ANGULARES

ROTACION ANGULAR -ROLL 𝜙
Rotación sobre el eje 𝑥, se obtiene al manipular las velocidades de los rotores 2 y 4, 
el par de rotores restantes permanece invariante en velocidad, al igual que el torque total. 
Se conoce como movimiento de Alabeo, hacia la izquierda y derecha.   

ROTACION ANGULAR -PITCH 𝜃
Rotación sobre el eje 𝑦, se obtiene al modificar las velocidades de los rotores 1 y 3, 
el par de rotores  restantes  permanece  invariante  en  velocidad,  al  igual  que  el  torque  total.  
Este  es  el  movimiento de cabeceo Pitch, hacia adelante y atrás. 
 
ROTACION ANGULAR -YAW (𝜓)
Rotación  sobre  el  eje 𝑧. Yaw  Movimiento  de  Guiñada.    
La  velocidad  de  los  rotores  1  y  3  se  aumenta, los rotores  2 y 4 se disminuyen para 
mover la orientación a la derecha. Esto produce un desbalance aerodinámico de los torques que 
permite que el cuadricóptero gire en su propio eje. Si todos los rotores mantienen la misma velocidad,
el torque total será cero; y si además el empuje total de la aeronave es igual a su peso,
esta se mantendrá en vuelo estacionario.
"""
